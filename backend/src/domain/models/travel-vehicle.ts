...
export class TravelVehicle {
  ...
  public calculate(): TravelVehicle {
    this.validateUnits();
    ...
    return new TravelVehicle(this.value, this.emissionsFactor, emissions);
  }

  private validateUnits(): void {
    if (this.value.unit !== Unit.KILOMETER_PER_YEAR) {
      throw new InvalidValueException(
        `Invalid unit for travel vehicle value: ${this.value.unit}`,
      );
    }

    if (this.emissionsFactor.unit !== Unit.KILOGRAMS_OF_CARBON_PER_KILOMETER) {
      throw new InvalidValueException(
        `Invalid unit for travel vehicle emissions factor: ${this.emissionsFactor.unit}`,
      );
    }
  }
}
